<div class="minibox">
	...
	<div id="entry" class="hide">
		<?php foreach(Entry::find('all', array('order' => 'source', 'conditions' => array('miniature_id = ?', $oGallery->id))) as $oEntries){
	...
	<div id="sources">
		<?php foreach(Source::find('all', array('order' => 'label', 'conditions' => array('miniature_id = ?', $oGallery->id))) as $oSources){
		?><a href="<?php echo $oSources->link ?>" target="_blank"><?php echo $oSources->label ?></a> | <?php }?> </div>
	<div id="stats">
		<?php foreach(Stat::find('all', array('order' => 'label', 'conditions' => array('miniature_id = ?', $oGallery->id))) as $oStats){
